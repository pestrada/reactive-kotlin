...
class FakeDb : Db {
    val userMap = mutableMapOf(
            ...
            3 to User(3, "John"),
            4 to User(4, "Carl"))
    val userPointsMap = mapOf(1 to 40, 2 to 23, 3 to 0, 4 to 5)

    override fun getAllUsers(): Flowable<User> = Flowable.fromIterable(userMap.values)

    override fun getUserById(id: Int): Single<User> {
        return getAllUsers()
                .filter { it.id == id }
                .firstOrError()
                .onErrorResumeNext { e -> Single.error(UserNotFound("Id: $id is not on the records.")) }
    }
